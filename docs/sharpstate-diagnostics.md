...
- **Using post-transition async work when you need transactional behavior.** `WhenExitedAsync(...)`, `WhenEnteredAsync(...)`, and `ReactAsync(...)` run after the transition already committed. If the work must finish before the new state becomes visible, keep it in `Invoke(...)` instead. See [Post-Transition Async Work](sharpstate-async.md).
